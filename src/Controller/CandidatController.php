...
public function register(Request $request, ValidatorInterface $validator, FileUploader $fileUploader): Response
{
    ...
    private function sendWelcomeEmail(Candidat $candidat): void
    {
        ...
        if (curl_errno($ch) || $httpCode >= 400) {
            // Pour enregistrer l'erreur, tu peux ajouter un logger
    ...
    public function login(Request $request): Response
    {
        if ($request->isMethod('POST')) {
            ...
            if ($candidat && $candidat->getPassword() === $password) {
                ...
                $script = "
                    <script>
                        localStorage.setItem('candidat_id', '{$candidat->getId()}');
                        localStorage.setItem('candidat_name', '".addslashes($candidat->getName())."');
